...
Add exactly one icon to each `##` header. Leave `###` and deeper headers plain — decorating sub-headers creates noise without structure.

**Icon vocabulary — pick by semantic meaning:**

| Icon | Use for |
|------|---------|
| 🧭 | Mental model, conceptual framing, orientation |
| 📌 | What X is, definitions, overview |
| 🔄 | Lifecycle, workflow, process flow |
| 📦 | Core objects, data model, structure |
| 🛠️ | Code patterns, instrumentation, setup |
| 📊 | Metrics, analytics, dashboards |
| 🔍 | Troubleshooting, debugging |
| ✅ | Checklist, verification steps |
| ⚠️ | Common misunderstandings, pitfall sections |
| 🔗 | Context propagation, connections, relationships |
| 🏷️ | Naming guidelines, identity, resources |
| 📤 | Exporters, transport, sending data |
| 🗄️ | Backends, storage |
| 🔀 | Collector pipeline, routing |
| 🎲 | Sampling |
| 📐 | Design rules, principles, decision points |
| 🗺️ | Minimal / baseline production shape |
| 🔌 | API, SDK, integration layers |
| 📡 | Signals overview (traces, metrics, logs) |
| 🌐 | Environment variables, deployment config |
| 🔒 | Security, privacy |
| 💬 | Prompts, prompt management |
| 📁 | Datasets, experiments, files |
| 🧪 | Evaluation, scoring, quality |
| 👤 | Sessions, users |
| 🔔 | Alerting, notifications |
| 🏗️ | Architecture overview |
| 🧩 | End-to-end examples, integration patterns |

**Callout blockquotes (1–3 per file, never more):**

Use `> ⚠️ **Watch out:** ...` for critical warnings already present in the text that a reader scanning for "how to use it" would easily skip — things that cause silent failures, data loss, or wasted hours in production.

Use `> 💡 **Key insight:** ...` for the single most non-obvious fact in a section — something that changes how you'd design or use the tool, not just a restatement of what was already said.

Keep both to one sentence. Don't add callouts to every section — if everything is highlighted, nothing is.
...
When a note set has **3+ independent files**, launch one subagent per file in parallel (all Agent tool calls in a single message). Each subagent writes its own `.md` file to disk following this skill's conventions, then returns a short confirmation — not the file contents.

Prompt each subagent with: the topic, target file path, audience, adjacent files for cross-linking, and a pointer to this SKILL.md. If the user wants external or current information, tell the subagent to use web search for its topic; otherwise it writes from native knowledge.

The main agent keeps: proposing structure, writing the root and directory READMEs, and verifying cross-references at the end.
...
2. **Create root README.md and directory READMEs** — Write these directly in the main agent; they're structural, not research-heavy.
3. **Write note files** — If 3+ independent files, launch subagents in parallel (see above). Otherwise write them directly.
4. **Verify cross-references** — Check all internal links point to files that exist.
...
- Don't use emojis decoratively. The only emojis allowed are: `1️⃣`–`9️⃣` for section numbering in intro files, `✅❌⚠️💡` for inline callout markers, and the approved icon vocabulary on `##` headers. No emojis on `###` or deeper headers, bullet points, table cells, or prose.
